...
const client = new TextAnalyticsClient(
  "YOUR_TEXT_ANALYTICS_ENDPOINT",
  new AzureKeyCredential("YOUR_TEXT_ANALYTICS_KEY")
);
...
app.get("/api/data", async (req, res) => {
  const querySpec = {
    query:
      "SELECT  c.tweet.created_at, c.tweet.id_str, c.tweet.text, c.tweet.user.screen_name, c.sentiment.sentiment FROM c ORDER BY c._ts DESC",
  };
...
app.get("/api/piechartdata", async (req, res) => {
  const piechartquerySpec = {
    query:
      "SELECT COUNT(1) AS TweetCount, c.sentiment.sentiment AS TweetSentiment FROM c GROUP BY c.sentiment.sentiment",
  };
...
app.get("/api/barchartdata", async (req, res) => {
  const barchartquerySpec = {
    query:
      "SELECT COUNT(1) AS TweetCount, c.tweet.user.screen_name AS TweetUser FROM c GROUP BY c.tweet.user.screen_name",
  };
